...
export class MenuManagementComponent implements OnInit , OnDestroy {
  ...
  isAddEditPanelOpen: boolean = false;
  ...
  private deleteMenuItem(id: any): void {
    ...
    this.service.deleteMenuItem(id).pipe(
      finalize(() => {
        this.tokenService.hide();
        this.isLoadingSubject.next(false);
      }),
  ...
  private mapToItems(data: any[]): Items[] {
    return data.map(item => {
      ...
      const takeAwayPriceAttr = item.attributes.find((a: any) => a.attributeName === 'takeAway');
      ...
      return {
        ...
        takeAwayPrice: takeAwayPriceAttr ? Number(takeAwayPriceAttr.attributeValue) : undefined,
